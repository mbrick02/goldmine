<?php
class Associated_store_items_and_store_item_colors extends Trongate {

    private $associated_modules = ['store_items', 'store_item_colors'];
    private $assoc_module = 'associated_store_items_and_store_item_colors';
    ...
    function _draw_association_info($token) {
        ...
        if ($data['calling_module'] == 'store_items') {
            $data['associated_records_singular'] = 'Store Item Color ';
            $data['associated_records_plural'] = 'store item colors';
        } else {
            $data['associated_records_singular'] = 'Store_item ';
            $data['associated_records_plural'] = 'store_items';            
        }
    ...
    function fetch($update_id) {
        ...
        if (is_numeric($comma_pos)) {
            ...
            $sql = '
                SELECT
                    associated_store_items_and_store_item_colors.id as recordId,
                    associated_module_name.id ,
                    CONCAT(xxxx) as identifierColumn 
                FROM
                    associated_module_name
                LEFT JOIN associated_store_items_and_store_item_colors ON associated_module_name.id = associated_store_items_and_store_item_colors.associated_module_name_id
                WHERE
                    associated_store_items_and_store_item_colors.calling_module_name_id = '.$update_id;
            ...
            $sql.= ' ORDER BY '.$associated_module_name.'.'.$identifier_columns[1];
            ...
            $sql = '
                ...
                $sql.= ' ORDER BY '.$associated_module_name.'.'.$identifier_column;
    ...
    function _build_sql_statement($identifier_column, $associated_module_name, $update_id) {
        ...
        if (is_numeric($comma_pos)) {
            ...
            $sql = '
                ...
                FROM
                '.$associated_module_name.'
                LEFT JOIN associated_store_items_and_store_item_colors                ON '.$associated_module_name.'.id = associated_store_items_and_store_item_colors.'.$associated_module_name.'_id 
                WHERE associated_store_items_and_store_item_colors.'.$associated_module_name.'_id IS NULL';
        ...
        if ($relationshipType == 'many to many') {
            ...
            $ditch = 'WHERE associated_store_items_and_store_item_colors.'.$associated_module_name.'_id IS NULL';
            $replace = 'WHERE associated_store_items_and_store_item_colors.'.$calling_module_name.'_id >0 ';
            $replace.= 'OR associated_store_items_and_store_item_colors.'.$calling_module_name.'_id IS NULL';
    ...
    function _get_associated_module_name($calling_module_name) {
        ...
        if ($calling_module_name == 'store_items') {
            $associated_module_name = 'store_item_colors';
        } else {
    ...
    function _prep_inbound($input) {
        ...
        if ($params['callingModule'] == 'store_item_colors') {
            $data['store_items_id'] = $params['selectedValue'];
            $data['store_item_colors_id'] = $params['update_id'];
        } else {
            $data['store_item_colors_id'] = $params['selectedValue'];
